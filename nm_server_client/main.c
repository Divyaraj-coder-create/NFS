...
void copy_parallely(char *src_ip, char *dest_ip, int src_port, int dest_port, char *src_path, char *dest_path, int src_server_number, int src_port_for_nm, int dst_port_for_nm)
{
    ...
    while (connect(src_sock, (struct sockaddr *)&src_addr, sizeof(src_addr)) < 0)
    {
        perror("[-]Connect error");
        // close(src_sock);
        // return;
    ...
    {
        printf("ggg\n");
        int n=recv(src_sock, buffer, sizeof(buffer), 0);
        if(n<=0)
        {
            continue;
        }
        ...
        usleep(1000);
        buffer[n]='\0';
        // Find the position of the "STOP" signal in the buffer
        char *stop_position = strstr(buffer, "STOP");
        if (stop_position != NULL)
        {
            // Null-terminate the buffer before the "STOP" signal
            *stop_position = '\0';

            // Send the portion of the buffer up to the "STOP" signal
            send(dest_sock, buffer, strlen(buffer), 0);

            // Send the "STOP" signal separately
        ...
        // Send the entire buffer if "STOP" is not found
        send(dest_sock, buffer, strlen(buffer), 0);
    }
